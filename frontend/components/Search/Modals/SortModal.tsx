...
import { setCurrentPage } from "@/store/slices/doctorsSlice";
  ...
  >
    ...
    >
      <Ionicons
        ...
        size={22}
        color={isSelected ? "#254EDB" : "#6b7280"}
      />
    </View>
    <ThemedText
...
const SortModal: React.FC = () => {
  ...
  const { sortModal } = useSelector((state: RootState) => state.modal);
  ...
  const handleSelectSort = (sort: "rating" | null) => {
    dispatch(setCurrentPage(1));
    dispatch(setFilters({ sort }));
    dispatch(closeModal(ModalType.SORT));
  };

  const handleClearSelection = () => {
    dispatch(setCurrentPage(1));
    dispatch(setFilters({ sort: null }));
    dispatch(closeModal(ModalType.SORT));
  };
            ...
            >
              ...
              <ScrollView className="p-4">
                <SortOption
                  ...
                  isSelected={filters.sort === "rating"}
                  onPress={() => handleSelectSort("rating")}
                />
                <SortOption
                  label="Sıralama yoxdur"
                  isSelected={filters.sort === null}
                  onPress={() => handleSelectSort(null)}
                />
              </ScrollView>
            </Animated.View>
          </TouchableWithoutFeedback>
